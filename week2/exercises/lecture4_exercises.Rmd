...
precision.bayes <- function(data, x, M, basis_type, delta, q) {
  ...
  return(list(test.y, sds, Qbayes, phi.test.x))
}
...
# generate a bunch of random variables between 0 and one as our test x,
# send to phix, solve for y(x)
draws <- mvrnorm(100, mu = colMeans(phi.test.x), Sigma = solve(sigma))
plot(data, col = 'red')

for (i in 1:nrow(draws)){
  lines(predict(splines::interpSpline(test.x, draws[i,])))
}
