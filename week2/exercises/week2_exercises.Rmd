...
$$\bar{y(\textbf{x})} = \sum_{n=1}^{N} q \phi(x)^{T} Q^{-1} \phi(x_{n})t_{n}$$

We use this to solve for $t_{n}$ and get

$\dfrac{\bar{y(\textbf{x})}}{\sum_{n=1}^{N} q \phi(x)^{T} Q^{-1} \phi(x_{n})t} = t_{n}$

We find the summation in the denominator (which is $k(x,x_{n})$) acts as a weight of the predictive distribution's mean at $\textbf{x}$ to produce $t_{n}$.
