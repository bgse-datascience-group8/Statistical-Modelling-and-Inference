...
legend(0, 0, legend = c("mle", "bayes", "bayes sng"), lty=c(1,1), lwd=c(2.5,2.5),col=c("green", "dodgerblue4", "orange"))
...
predict.sng.params <- function(test.x) {
  ...
  #   This looked crazy and I'm not so sure about using q in the equation above, so trying to use K as our fitter.
  #   phi %*% w.random
  # And make predictions for t using K
  fittedt <- c()
  if (nrow(phi) == length(data$t)) {
    Kprime <- tau * phi %*% solve(Dprime + tau * t(phi) %*% phi) %*% t(phi)
    fittedt <- Kprime %*% data$t 
  }
  ...
  for (n in 1:nrow(test.y)) {
    ...
    test.y[n,"test.y"] <- rt.scaled(
      n = 1,
      mean = t(xphix) %*% params$bayes.params,
      sd = 1/sqrt(params$bayes.sng.aprime/params$bayes.sng.bprime * F),
      df = params$bayes.sng.aprime * 2)
  }
  return(list(predictions = test.y, fittedt = fittedt))
}

plot(data, col = 'chocolate1', main = "Fit t with random draws of the precision")
for (n in 1:100) {
  fittedt <- predict.sng.params(data$x)$fittedt
  lines(predict(splines::interpSpline(data$x, fittedt)), col = 'dodgerblue4')
}
points(data, col = 'chocolate1', pch = 21, bg = 'chocolate1')
